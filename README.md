...
| 🛡️ 负向语境保护 | 自动降低科普、辟谣、打假、监管和劝阻购买等内容的误判风险 |

### ⚙️ 保护策略

| 功能 | 说明 |
|------|------|
| 🎚️ 多档保护强度 | 按平台（抖音/快手）和内容类型（短视频/直播）独立配置敏感等级 |
...
| ⏱️ 智能触发 | 仅在目标应用前台时激活，离开、锁屏或屏幕关闭时自动暂停 |
...
| 模块 | 说明 |
|------|------|
| 👁️ 视觉模型 | 输入 `float32[1,3,416,192]`，输出页面类型、营销强度、五个健康领域标签和老年指向的概率 |
...
> ⚠️ **注意：** 这是本地辅助工具，不是医疗、法律或金融决策系统。模型可能在新界面、低质量画面、遮挡画面和健康科普内容中产生误判。自动划走功能应由用户自行开启并承担相应风险。
...
| 层次 | 技术 | 用途 |
|------|------|------|
| 👁️ 视觉模型 | MobileNetV3-Small (PyTorch → LiteRT) | 画面分析：页面类型、营销强度、健康领域、老年指向 |
...
守目人目前是一个人在有限条件下完成的项目，模型和软件都有很大的改进空间。欢迎任何形式的贡献：

- 🧠 **模型改进** — 优化视觉模型、文本分类器和融合策略，提高识别准确率，降低误判率。训练工具和评估数据已在仓库中提供，可以基于现有流水线继续迭代
- 💻 **软件开发** — 修复 Bug、优化性能、增加新功能、适配更多平台和设备
- 📊 **数据标注** — 帮助标注更多真实场景的视频帧数据，扩充训练集
- 🐛 **问题反馈** — 使用中遇到的任何问题或建议，欢迎提交 Issue
- 📖 **文档完善** — 改进使用教程、翻译、补充说明

> 💡 **想参与？** Fork 本仓库，进行修改后提交 Pull Request。如果你对模型训练感兴趣，可以参考 [模型仓库](https://huggingface.co/AvalonskyAfar/KeepersEye-1) 中的详细说明和训练流水线。
...
| 🙏 所有贡献者 | 参与数据采集、人工标注、复核、真机测试和问题排查 |
